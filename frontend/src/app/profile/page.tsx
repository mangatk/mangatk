...
//    FaBook, FaDownload, FaCommentDots, FaReply, FaCheck, FaTrophy, FaLock, FaEdit, FaTimes,
// } from 'react-icons/fa';
...
//    const { user, login, logout, isLoading: authLoading, getAuthHeaders, updateLocalUserName, refreshUserProfile  } = useAuth();
...
//    //       if (!res.ok) {
//    //          throw new Error(data.error || data.detail || 'فشل حفظ الاسم');
//    //       }

//    //       toast.success('تم تحديث الاسم الظاهر بنجاح');
//    //    } catch (err: any) {
//    //       console.error(err);
//    //       toast.error(err.message || 'حدث خطأ أثناء حفظ الاسم');
//    //    } finally {
...
//          // Update name instantly in all frontend places
//          updateLocalUserName(
...
//          // Optional extra sync from backend
...
//                               {t('welcomeUser')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">{user.name}</span> 👋
...
//                               هذا الاسم هو الذي يظهر في حسابك والتعليقات بدلاً من اسم Google/Auth0
//                            </p>
//                            </div>
...
//                                           <p className="text-white text-xs font-bold mb-4 opacity-60 uppercase tracking-widest text-center">{t('badgeColor')}</p>
...
//                                              />
//                                              <span className="text-white/60 text-xs font-medium">{t('customColor')}</span>
//                                           </div>
//                                           {badgeColor && (
//                                              <button
//                                                 onClick={() => {
...
//                            <p className="text-blue-100/70 text-sm mt-3 font-medium hidden md:block">{t('commandCenter')}</p>
//                         </div>
//                      </div>
...
//                               <p className="text-purple-100 text-sm font-bold mb-1 opacity-80 uppercase tracking-wider">{t('smartPoints')}</p>
//                               <div className="flex items-end gap-3 mt-2">
//                                  <h3 className="text-5xl font-black tracking-tighter drop-shadow-lg">{currentPoints}</h3>
//                                  <span className="text-sm font-bold bg-white/20 backdrop-blur-sm px-3 py-1 rounded-xl mb-2 shadow-inner border border-white/20">🪙 +1/{t('pointsPerChap')}</span>
//                               </div>
//                            </div>
//                         </div>
...
//                                 <FaClock /> {t('totalReadingTime')}
//                               </p>
//                               <div className="flex items-baseline gap-2 mt-3 bg-black/10 backdrop-blur-sm rounded-2xl p-3 w-max border border-white/10">
//                                  <span className="text-3xl font-black tracking-tight">{readingTime.days}</span><span className="text-xs opacity-80 font-bold pr-1">{t('day')}</span>
//                                  <span className="text-3xl font-black tracking-tight ml-4">{readingTime.hours}</span><span className="text-xs opacity-80 font-bold pr-1">{t('hour')}</span>
//                                  <span className="text-3xl font-black tracking-tight ml-4">{readingTime.minutes}</span><span className="text-xs opacity-80 font-bold pr-1">{t('minute')}</span>
//                               </div>
//                            </div>
//                         </div>
...
//                               <p className="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mt-1">{t('epicFavorites')}</p>
//                            </div>
//                         </Link>
...
//                               <p className="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mt-1">{t('chaptersRead')}</p>
//                            </div>
//                         </div>
...
//                               <p className="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mt-1">{t('downloads')}</p>
//                            </div>
//                         </div>
...
//                               <p className="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mt-1">{t('chaptersAvailable')}</p>
//                            </div>
//                         </div>
//                      </div>
...
//                               <FaTrophy className="text-yellow-500" /> {t('achievementsHall')}
//                            </h2>
...
//                                           {isSecret && !isUnlocked ? t('secretAch') : achDesc}
//                                        </p>
//                                     </div>
...
//                                  {replyNotifications.length} {t('newBadge')}
//                               </span>
//                            )}
//                         </h2>
//                      </div>
...
//                               </div>
//                               <p className="text-sm font-bold text-gray-500">{t('inboxEmpty')}</p>
//                            </div>
//                         ) : (
...
//                                        <p className="opacity-70 text-[10px] mb-1 font-bold">{t('yourPrevComment')}</p>
...
//                                              {t('chapterPrefix')} {notif.chapterId}
//                                           </Link>
//                                        </div>
...
//                                                    <FaReply /> {t('sendReply')}
//                                                 </button>
//                                                 <button onClick={() => setActiveReplyId(null)} className="px-5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors shadow-sm">{t('cancel')}</button>
//                                              </div>
//                                           </div>
//                                        ) : (
...
import { formatReadingTime } from '@/hooks/useReadingTime';
import { useAchievements } from '@/hooks/useAchievements';
import { ALL_ACHIEVEMENTS, RARITY_COLORS } from '@/data/achievements';
import { ProxyImage } from '@/components/ProxyImage';
import {
   FaHeart, FaHistory, FaSignOutAlt, FaClock,
   FaBook, FaDownload, FaCommentDots, FaReply, FaCheck, FaTrophy, FaLock, FaEdit, FaTimes,
} from 'react-icons/fa';
import Link from 'next/link';
import { Header } from '@/components/Header';
...
export default function ProfilePage() {
   const { user, login, logout, isLoading: authLoading, getAuthHeaders, updateLocalUserName } = useAuth();
   const { bookmarks, history } = useStorage();
   const { unlockedIds } = useAchievements();
   const { t } = useLanguage();
   const router = useRouter();
   ...
   useEffect(() => {
      ...
      if (user) {
         ...
         setRecentFavCount(Math.min(bookmarks.length, 5));
         setDownloadedCount(15);
            ...
            <div className="container mx-auto px-4">
               ...
               <div className="relative mb-12 rounded-[2rem] overflow-hidden shadow-2xl group border border-gray-200 dark:border-gray-800">
                  ...
                  <div className="relative p-6 md:p-10 flex flex-col md:flex-row items-center md:items-end justify-between gap-6">
                        ...
                        {(() => {
                           ...
                           return (
                              <div className={`w-28 h-28 md:w-32 md:h-32 rounded-[2rem] bg-gradient-to-br ${rarityGrad} flex items-center justify-center text-5xl font-black text-white shadow-2xl relative transform transition-transform hover:-translate-y-2 hover:shadow-blue-500/50 border-4 border-white/20 overflow-hidden`}>
                                 ...
                                 {equippedTitle && (
                                    <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-full bg-green-500 border-4 border-gray-900 shadow-lg animate-pulse z-10 hidden md:block"></div>
                                 )}
                              </div>
                           );
                        })()}
                                    ...
                                    <div
                                       className={`text-sm font-black ${
                                          badgeColor
                                             ...
                                             : `bg-gradient-to-r ${RARITY_COLORS[equippedTitle.rarity as keyof typeof RARITY_COLORS] || 'from-gray-300 to-gray-400'} bg-clip-text text-transparent drop-shadow-sm`
                                       }`}
                                    >
                                       {equippedTitle.title}
                                    </div>
                     ...
                     <button
                        onClick={logout}
                        className="px-5 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 backdrop-blur-md rounded-2xl transition-all flex items-center gap-2 font-bold shadow-lg hover:shadow-red-500/20 hover:-translate-y-1 group"
                     >
                           ...
                           {(apiAchievements !== null ? apiAchievements : ALL_ACHIEVEMENTS).map((ach: any) => {
                              ...
                              return (
                                 <div
                                    ...
                                    className={`relative p-5 rounded-2xl border flex flex-col items-center text-center transition-all duration-500 group overflow-hidden ${
                                       isUnlocked
                                          ? `cursor-pointer bg-white dark:bg-gray-800 shadow-lg hover:-translate-y-2 hover:shadow-2xl ${
                                             isEquipped ? 'border-yellow-400 ring-4 ring-yellow-400/20 shadow-yellow-500/30' : 'border-gray-100 dark:border-gray-700 hover:border-blue-500/50 hover:shadow-blue-500/10'
                                          }`
                                          : 'bg-gray-50 dark:bg-gray-900 border-gray-100 dark:border-gray-800 opacity-60 grayscale cursor-not-allowed'
                                    }`}
                                 >
                                    {isUnlocked && (
                                       <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/40 to-white/0 translate-y-full group-hover:-translate-y-full transition-transform duration-1000 ease-in-out z-0 opacity-20"></div>
                                    )}
                                    ...
                                    <div className={`relative z-10 w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-4 transition-all duration-500 ${
                                       isUnlocked
                                          ? `bg-gradient-to-br ${rarityColor} text-white shadow-xl shadow-current/30 group-hover:scale-110 group-hover:rotate-12`
                                          : 'bg-gray-200 dark:bg-gray-700 text-gray-400'
                                    }`}>
                                          ...
                                          {activeReplyId === notif.replyId ? (
                                             <div className="mt-3 animate-in fade-in slide-in-from-top-2">
                                                ...
                                                <div className="flex gap-2">
                                                   ...
                                                   <button onClick={() => setActiveReplyId(null)} className="px-5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors shadow-sm">
                                                      {t('cancel')}
                                                   </button>
                                                </div>
                                             </div>
